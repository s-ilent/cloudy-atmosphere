...
{
    [CreateAssetMenu(fileName = "New Atmosphere", menuName = "Atmosphere", order = 1)]
    public class Atmosphere : ScriptableObject
    {
        ...
        private void Precompute()
        {
            ...
            m_model.BindToMaterial(m_skyboxMaterial);
        ...
        {
            m_skyboxMaterial.SetFloat("exposure", UseLuminance != LUMINANCE.NONE ? Exposure * 1e-5f : Exposure);
            m_skyboxMaterial.SetVector("earth_center", new Vector3(0.0f, -kBottomRadius / kLengthUnitInMeters, 0.0f));
            m_skyboxMaterial.SetVector("sun_size", new Vector2(Mathf.Tan(kSunAngularRadius), Mathf.Cos(kSunAngularRadius)));
            ...
            m_skyboxMaterial.SetVector("white_point", new Vector3((float)white_point_r, (float)white_point_g, (float)white_point_b));
        }
